 ...
 */
function getJSXExpressionContainerPropValue(expression: Record<string, any>): any {
  if (expression.type === 'ArrayExpression') {
    return expression.elements.map((element: Record<string, any>) => getJSXExpressionContainerPropValue(element));
  }

  if (expression.type === 'ObjectExpression') {
    const attr: Record<string, any> = {};

    expression.properties.forEach((property: Record<string, any>) => {
...
function getProps(attributes: Record<string, any>) {
  ...
  attributes.forEach((attr: Record<string, any>) => {
    if (attr.value === null) {
...
function reduceAstNode(
  oldNode: Record<string, any>[],
  currentNode: {
    type: string;
    openingElement: Record<string, any>;
    children?: any[];
  }
) {
  ...
  if (currentNode.type === 'JSXElement') {
    element = {
      $componentName: currentNode.openingElement.name.name,
 ...
 * }
 */
export default function getJsxCodeAst(codeStr: string) {
  try {
    const rawAst = babelParser.parse(codeStr, {
      plugins: ['jsx'],
    });

    return reduceAstNode([], (rawAst.program.body[0] as any).expression);
  } catch (error) {
